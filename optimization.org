...
* Host machine setup
We will want to ssh to =tune= to connect to the tune VM machine.  This
will require some configuration on the host machine (or you are edit
"=tune=" in the file:src/host-test.sh script).

1. Edit file:/etc/hosts so that "=tune=" redirects to "=localhost="
   changing the =localhost= line to something like the following.
   : 127.0.0.1       localhost.localdomain   localhost your-hostname tune

2. Edit your file:~/.ssh/config so that connections to =tune= use port
   2222 and connect as bacon by adding something like the following.
   : Host tune
   : Port 2222
   : User bacon
...
To run all overlays in their own screen windows run;
#+begin_src sh :tangle src/startup-vms
  remotes=""
  pushd overlays/
  for i in $(seq 46);do
      I=$(printf "%0.2d" i);
      screen -X screen -t $I-VM \
          qemu-kvm -hda $I.cqow -nographic -m 1G \
          -cdrom ../data/debian-5010-amd64-netinst.iso \
          -net nic -net user,hostfwd=tcp:127.0.0.1:30$I-:22
      remotes="$remotes \"-p 30$I bacon@localhost\""
  done
  cat <<EOF > REMOTES
  #!/bin/sh
  REMOTES=($remotes)
