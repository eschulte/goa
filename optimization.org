...
3. Now install the Graphite and Parsec dependencies,
   #+begin_src sh
     ...
     # because graphite expects this older version
...
4. Start up the virtual machine with network access so we can connect
   with ssh -- allowing copy-paste from the local machine, use of a
   normal terminal, and other niceties (note: you may also use the
   =bin/run-vm= script distributed in this repository).
   #+begin_src sh
...
1. Log into =tune= as =bacon=.
   ...
   #+begin_src sh
     ...
     EOF
   #+end_src
   run =make CXX=g++-4.3= (if you get a =libtool= error, then run
   =make= a second time w/o the =CXX= part) to build Graphite.
