...
2. We will use the file:src/limit.c test harness to protect our
   ...
   #+begin_src sh
     ...
     ssh -p 2222 root@localhost gcc -o bin/limit bin/limit.c
   #+end_src
...
| run                  | git-tag            | results-dir                               |
|----------------------+--------------------+-------------------------------------------|
| First Serial Run     | first-serial-run   | ~/research/epr/optimize/results/first-run |
| First Parallel Run   | first-parallel-run | ~/research/epr/opt-real/results/first-run |
| Parallel Runtime Run | pll-runtime-run    | ~/research/epr/opt-real/results/pll-2     |

#+name: results-summary
#+begin_src emacs-lisp :exports results
  (let ((fields (list "git-tag" "results-dir")))
    (append (list (cons "run" fields) 'hline)
            (save-restriction
...
** STARTED First Serial Run
   :PROPERTIES:
   :git-tag:  first-serial-run
   :results-dir: ~/research/epr/optimize/results/first-run
...
** CANCELED First Parallel Run
   ...
   :results-dir: ~/research/epr/opt-real/results/first-run
...
** STARTED Parallel Runtime Run
   ...
   :results-dir: ~/research/epr/opt-real/results/pll-2
...
#+begin_src lisp
  ...
  (setf *pop* (restore "../results/pll-2/biased-pop-113.store"))
  (setf *note-level* 1)
  (advise-thread-pool-size 46)
  (loop for n from 113 to 300 do (note 1 "saving population ~d" n)
