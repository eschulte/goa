...
8. To install Parsec, download and unpack the tarball from
...
9. Add Parsec and Graphite environment variables to the =~/.bashrc=.
   #+begin_src sh
     cat <<EOF >> ~/.bashrc
     #####################################################################
     ## Custom Environment Variables for Graphite and Parsec
     export GRAPHITE_HOME=$HOME/graphite
     export PARSEC_HOME=$HOME/parsec-3.0
     
     if [ -z "${PATH}" ]; then
       export PATH=${PARSEC_HOME}/bin
     else
       export PATH=${PATH}:${PARSEC_HOME}/bin
     fi
     
     # Append `man/' directory to MANPATH
     if [ -z "${MANPATH}" ]; then
       export MANPATH=${PARSEC_HOME}/man
     else
       export MANPATH=${MANPATH}:${PARSEC_HOME}/man
     fi
     
     # Add directory with hooks library to library search path
     if [ -z "${LD_LIBRARY_PATH}" ]; then
       export LD_LIBRARY_PATH="${PARSEC_HOME}/pkgs/libs/hooks/inst/${PARSECPLAT}/lib"
     else
       export LD_LIBRARY_PATH="${LD_LIBRARY_PATH}:${PARSEC_HOME}/pkgs/libs/hooks/inst/${PARSECPLAT}/lib"
     fi
     #####################################################################
     EOF
   #+end_src

10. Build parsec following these instructions adopted from [[https://github.com/mit-carbon/Graphite/wiki/Adding-and-Running-Applications#Running an application from the PARSEC benchmark suite][Running an
    application from the PARSEC benchmark suite]].

11. At this point the virtual machine is usable for running
...
[[https://github.com/mit-carbon/Graphite/wiki/Adding-and-Running-Applications#Running an application from the PARSEC benchmark suite][Running an application from the PARSEC benchmark suite]]

Set the environment variables =${PARSEC_HOME}= and =${GRAPHITE_HOME}=
to point to the root of the PARSEC and Graphite source directories
respectively.

: $ cd ${PARSEC_HOME} 

Next, start building the tools in PARSEC using the gcc configuration
file and populate the benchmark directories with a copy of the gcc
build configuration:

: $ parsecmgmt -a build -p tools
: $ touch config/parsec.conf
: $ bldconfadd -n graphite -c gcc -f

Next, use the configuration file located here to build the
...
Edit =graphite.bldconf= to set the GRAPHITE_HOME variable to the root
of your Graphite source tree.

Had to add
: build_inplace="TRUE"
to my =graphite.bldconf= file and then explicitly symlink to it from
the application directories.
#+begin_src sh
  for app in $(ls pkgs/apps);do
      cd pkgs/apps/$app/parsec/
      ln -s ../../../../config/graphite.bldconf 
      cd ../../../../
  done
  for app in $(ls pkgs/kernels);do
      cd pkgs/kernels/$app/parsec/
      ln -s ../../../../config/graphite.bldconf 
      cd ../../../../
  done
#+end_src

Next build the PARSEC applications and kernels using the
=graphite.bldconf= configuration file:

: $ ./bin/parsecmgmt -a build -p apps -c graphite
: $ ./bin/parsecmgmt -a build -p kernels -c graphite

Then return to the Graphite home directory.

: $ cd ${GRAPHITE_HOME}

Set the PARSEC_HOME variable in the file =tests/Makefile.parsec= to
point to the source of the PARSEC source tree.

Run =make [app]_parsec= to simulate the =[app]= with Graphite using
the parameters specified in =tests/Makefile.parsec=.
